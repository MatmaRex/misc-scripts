...
require 'D:\Moje dokumenty\BARTOS~1\Dropbox\Programowanie i HTML\ruby\Sunflower\lib\sunflower\listmaker.rb'
...
ex = ['Uzbrojenie morskie']

if File.exist? 'list-marshal'
	list = Marshal.load File.open('list-marshal','rb'){|f| f.read}
else
	list = s.make_list 'category-r', 'Kategoria:Okręty'
	exclude = ex.map{|a| s.make_list 'category-r', 'Kategoria:'+a}.flatten
...
5.times do
	threads<<Thread.new do
		begin
			until list.empty?
				name = list.shift
				images, interwikimap, already = get_images 'pl', name
				
				results[name]=[images, interwikimap, already]
				puts list.length
			end
		rescue
			puts $!, $!.backtrace
