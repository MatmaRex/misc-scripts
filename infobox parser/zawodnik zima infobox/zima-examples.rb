...
api_endpoint = 'action=query&list=embeddedin&eilimit=max&eititle=Szablon:Zawodnik%20zima%20infobox'
titles = []
...
titles.each_slice(50).with_index do |titles, i|
	...
	test_page = Page.new "User:Matma Rex/zawodnik zima - testy/#{i}"
	test_page.text = ''
	...
	titles.each_with_index do |title, i|
		...
		new = ib.pretty_format(opts).sub('Zawodnik zima infobox', 'Zawodnik zima infobox/nowy')
		
		new.gsub! '{{subst:PAGENAME}}', title
		test_page.text += (
			"== [[#{title}]] ==\n" +
...
test_page = Page.new "User:Matma Rex/zawodnik zima - testy"
test_page.text = (0..titles.length/50).map{|i| "# [[User:Matma Rex/zawodnik zima - testy/#{i}]]" }.join("\n")
test_page.save
